...
from vit_masked_sphere import DGCNN_voxel_reshape
import numpy as np
...
def _init_():
    if not os.path.exists('/data4/zb/model/model_nyu2/1449_6cls/sphere200_voxel0.2_dim728_cluster512_transformer_using_pretrain'):
        os.makedirs('/data4/zb/model/model_nyu2/1449_6cls/sphere200_voxel0.2_dim728_cluster512_transformer_using_pretrain')



def train(args, io):

    train_loader = DataLoader(nyu2_1449_sphere('train'), num_workers=8,
    ...
    if args.model == 'pointnet':
        ...
        model = DGCNN_voxel_reshape(
            ...
            max_len = 728,                                                 #########################   how many are the most not empty voxels
            depth=6,
            heads=16,
    ...
    pretrain_voxel_dgcnn = torch.load(
        '/data4/zb/model/model_nyu2/1449_6cls/pretrain_for_voxel_dgcnn_by_vfh_label512/model_199.t7')
    ...
    for epoch in range(0, args.epochs, 1):
        ...
        outstr = 'Train %d, lr %6f, loss: %.6f, train acc: %.6f, train avg acc: %.6f' % (epoch,
                                                                                 ...
                                                                                 metrics.balanced_accuracy_score(
                                                                                     train_true[batch_size:], train_pred[batch_size:]))
        io.cprint(outstr)
        torch.save(model.state_dict(), '/data4/zb/model/model_nyu2/1449_6cls/sphere200_voxel0.2_dim728_cluster512_transformer_using_pretrain/model_transformer_masked_%s.t7'%(epoch))
...
def test(args, io):
    ...
    for iii in range(0,200,1):   #### test the last 50 models
        model_path = '/data4/zb/model/model_nyu2/1449_6cls/sphere200_voxel0.2_dim728_cluster512_transformer_using_pretrain/model_transformer_masked_%s.t7'%(iii)
        model.load_state_dict(torch.load(model_path))
        model = model.eval()
        test_acc = 0.0
        count = 0.0
...
if __name__ == "__main__":
    ...
    io = IOStream('/data4/zb/result_log/NYU2_1449_res0.2_sphere200_voxel728_dim512_transformer_masked.log')
    io.cprint(str(args))
