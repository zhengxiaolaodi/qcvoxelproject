...
from Model_40_data import modelnet40_40cls_10percls_voxel_pcd_vfh_cluster_label
from Model_40_voxel_dgcnn_pretrain import DGCNN_voxel_reshape
import numpy as np
...
def _init_():
    if not os.path.exists('/data4/zb/model/modelnet40_res0.5/pretrain/pretrain_for_voxel_dgcnn_by_gasd_cluster_label256'):
        os.makedirs('/data4/zb/model/modelnet40_res0.5/pretrain/pretrain_for_voxel_dgcnn_by_gasd_cluster_label256')


def train(args, io):

    train_loader = DataLoader(modelnet40_40cls_10percls_voxel_pcd_vfh_cluster_label(), num_workers=8,
    ...
    if args.model == 'pointnet':
        model = PointNet_voxel_reshape().to(device)
    ...
    model = torch.nn.DataParallel(model, device_ids=[0, 2, 3, 5,6])
    ...
    for epoch in range(0, args.epochs, 1):
        ...
        outstr = 'Train %d, lr %6f, loss: %.6f, train acc: %.6f, train avg acc: %.6f' % (epoch,
                                                                                 ...
                                                                                 metrics.balanced_accuracy_score(
                                                                                     train_true[batch_size:], train_pred[batch_size:]))
        io.cprint(outstr)
        torch.save(model.state_dict(), '/data4/zb/model/modelnet40_res0.5/pretrain/pretrain_for_voxel_dgcnn_by_gasd_cluster_label256/model_pretrain_for_voxel256_gasd_%s.t7'%(epoch))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--voxel_num', type=int, default=59,  ###################
                        help='num of points to use')
    ...
    io = IOStream('/data4/zb/result_log/run_model40_res0.5_delno_pretrain_by_gasd_cluster_label256.log')
    io.cprint(str(args))
