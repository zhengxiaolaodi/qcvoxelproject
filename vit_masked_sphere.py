...
from data_voxel_reshape import nyu2_1449_sphere
...
def positional_adding(x, voxel_sequence, cloud_len_list, voxel_radius, d_model, max_len):
    """
    ...
    for i in range(batch_size):
        ...
        position = (voxel_sequence[i]/voxel_radius[i]).unsqueeze(1)
        #print(position.shape,111111111)
...
class Attention(nn.Module):
    ...
    def forward(self, x, voxel_sequence, cloud_len_list):
        ...
        mask_map = get_attn_pad_mask(self.max_len+1, voxel_sequence, cloud_len_list)
    ...
    def __init__(self, dim, max_len, depth, heads, dim_head, mlp_dim, dropout = 0.):
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, Attention(dim, max_len = max_len, heads = heads, dim_head = dim_head, dropout = dropout)),
...
class DGCNN_voxel_reshape(nn.Module):
    def __init__(self, num_classes, dim, max_len, depth, heads, mlp_dim, pool = 'cls', channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0.):
        super().__init__()
        self.voxel_cls = dim
        self.max_len = max_len
        ...
        self.transformer = Transformer(dim, max_len, depth, heads, dim_head, mlp_dim, dropout)
    ...
    def forward(self, input, cloud_len_list, voxel_sequence, voxel_radius):
        ...
        xx = torch.zeros(len_cloud, self.max_len, self.voxel_cls).cuda()  # xx means voxel features                  ######################
        start_num = 0
        ...
        x = positional_adding(x, voxel_sequence, cloud_len_list, voxel_radius, d_model=self.voxel_cls, max_len = self.max_len)             ########################
        x = self.dropout(x)
...
if __name__ == '__main__':
    v = DGCNN_voxel_reshape(
        num_classes=6,
        dim=512,     ##### the voxel feature dimension
        max_len = 728,
        depth=6,
        heads=16,
    ...
    model = v.cuda()
    model = torch.nn.DataParallel(model, device_ids=[0, 1, 3, 5,7])
    ##############   加载预训练的voxel dgcnn
    # model_dict = model.state_dict()
    # pretrain_voxel_dgcnn = torch.load('./model/pretrain_for_voxel_dgcnn_by_vfh_cluster_label/model_pretrain_for_voxel380_nomean_199.t7')
    ...
    test_loader = DataLoader(nyu2_1449_sphere('test'), num_workers=8,
    ...
    for i, (data, label, cloud_len_list, voxel_seqence, voxel_radius) in enumerate(test_loader):
        """
        ...
        voxel_radius = torch.tensor(voxel_radius, dtype=torch.float, requires_grad=False).cuda()
        out = model(data, cloud_len_list, voxel_seqence, voxel_radius)
        print(out.shape)
        if i > 0:
            print(out.shape)
            print('------------------------')
