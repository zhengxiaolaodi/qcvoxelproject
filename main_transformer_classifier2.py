...
from model_nyu2_transformer_classifier2 import DGCNN_voxel_reshape
import numpy as np
...
def _init_():
    if not os.path.exists('./model/nyu2_crop_1.6_2_voxel0.2_downsmp_to220_transformer'):
        os.makedirs('./model/nyu2_crop_1.6_2_voxel0.2_downsmp_to220_transformer')
    if not os.path.exists('./model/nyu2_crop_1.6_2_voxel0.2_downsmp_to220_transformer/model_n0_knn_voxel_sequence_vcls760'):
        os.makedirs('./model/nyu2_crop_1.6_2_voxel0.2_downsmp_to220_transformer/model_n0_knn_voxel_sequence_vcls760')
...
def train(args, io):
    ...
    model = torch.nn.DataParallel(model, device_ids=[0, 1, 2, 3, 4, 5, 7 ])
    ...
    # for pre_epoch in range(105):
    #     scheduler_steplr.step()
    # model_path = './model/nyu2_crop_1.6_2_voxel0.2_downsmp_to220_transformer/model_n0_knn_voxel_sequence_vcls760/model_transformer_104.t7'
    ...
    for epoch in range(args.epochs):
        ...
        outstr = 'Train %d, lr %6f, loss: %.6f, train acc: %.6f, train avg acc: %.6f' % (epoch,
                                                                                 ...
                                                                                 metrics.balanced_accuracy_score(
                                                                                     train_true[batch_size:], train_pred[batch_size:]))
        io.cprint(outstr)
        torch.save(model.state_dict(), './model/nyu2_crop_1.6_2_voxel0.2_downsmp_to220_transformer/model_n0_knn_voxel_sequence_vcls760/model_transformer_%s.t7'%(epoch))
...
def test(args, io):
    ...
    for iii in range(95):   #### test the last 50 models
        model_path = './model/nyu2_crop_1.6_2_voxel0.2_downsmp_to220_transformer/model_n0_knn_voxel_sequence_vcls760/model_transformer_%s.t7'%(iii)
        model.load_state_dict(torch.load(model_path))
        model = model.eval()
        test_acc = 0.0
        count = 0.0
...
if __name__ == "__main__":
    ...
    parser.add_argument('--voxel_cls', type=int, default=760, metavar='N',     ##################
    ...
    io = IOStream('./result/run_no0knn_transformer2.log')
    io.cprint(str(args))
