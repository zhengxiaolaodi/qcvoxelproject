...
path="/data4/zb/qc/SUNRGBD_train_0.4"
f = h5py.File(path, 'r')
...
voxel_coods = f['voxel_coods'][:].astype('int64')
f.close()
model=FCGF_spconv
for i in range(data.shape[0]):
    feat=data[i]
    cood=voxel_coods[i]
    y=feat.reshape([feat.shape[0]*feat.shape[1],-1])
    spatial_shape=np.max(y,axis=-1)
    batch_size=1
    feat=feat.reshape([feat.shape[0]*feat.shape[1],-1])
    sinput= spconv.SparseConvTensor(feat,cood,spatial_shape,batch_size)
    result=model(sinput)